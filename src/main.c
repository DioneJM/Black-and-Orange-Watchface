...
static TextLayer *text; //The text displayed in the bottom right corner
...
static void update_time(){
  ...
  int min = tick_time->tm_sec;
  if(min % 2 ==0){text_layer_set_text(text, "Pebble");}
  else if (min %2 == 1){text_layer_set_text(text, "Time");}
...
static void main_window_load(Window *window) {
  ...
  // Text Display
  text = text_layer_create(GRect(bounds.size.w * 0.4 + 5, bounds.size.h * 0.9, 0.8 * bounds.size.w, 50));
  
  
  // Background
  text_layer_set_background_color(background_first_half, GColorOrange);
  text_layer_set_background_color(background_second_half, GColorBlack);
  ...
  text_layer_set_text_color(t_time_layer, GColorOrange);
  text_layer_set_text(t_time_layer, "00:00");
  text_layer_set_font(t_time_layer, fonts_get_system_font(FONT_KEY_BITHAM_42_LIGHT));
  text_layer_set_text_alignment(t_time_layer, GTextAlignmentCenter);
  
  // Text display
  text_layer_set_background_color(text, GColorClear);
  text_layer_set_text_color(text, GColorLightGray);
  text_layer_set_font(text, fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD));
